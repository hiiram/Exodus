  ...
  it('setup without complaining.', function (done) {
    Mongosess.connect(MongoOpts, function () {
      server = http.createServer(function (req, res) {
        ...
        switch (url.pathname) {
          case '/set':
            return res.session.set(url.query, function (err, result) {
              if (err) throw err;

              res.session.get(function (err, session) {
                if (err) throw err;
          ...
          case '/get':
            return res.session.get(url.query.prop, function (err, session) {
              if (err) throw err;

              session = session || {error: 'Not found.'};
          ...
          case '/del':
            ...
            params.push(function (err, result) {
              ...
              res.session.get(function (err, session) {
                res.send({
                  session: session,
                  deleted: params.slice(0, params.length-1)
                })
              })
            })
          ...
          case '/expire':
            res.session.expire()
            return res.session.get(function (err, session) {
              if (err) throw err
          ...
          case '/end':
            return res.session.end(function (err) {
              if (err) throw err;
